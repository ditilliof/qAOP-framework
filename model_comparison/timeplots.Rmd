...
#For this to work, you need to manually copy the "fitqAOPX_datafromY.rds" files, together with "artdata_qAOPY.csv", in the fits folder
```{r}
...
data_stanA <- process_data(csv_data_list[["artdata_qAOPA.csv"]]%>%select(-1))
data_stanB <- process_data(csv_data_list[["artdata_qAOPB.csv"]]%>%select(-1))
data_stanC <- process_data(csv_data_list[["artdata_qAOPC.csv"]]%>%select(-1))
```
