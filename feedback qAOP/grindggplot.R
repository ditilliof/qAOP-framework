...
sizeLegend <- 0.5    # legend size is 75% of the default value in R
...
# Add this to the grind.R script

bifurcation_data <<- data.frame(x = numeric(0), y = numeric(0), stable = logical(0)) # Global variable to store bifurcation data

continue <- function(state=inistate, parms=pars, odes=feedbackqAOP, step=0.01, x=1, y=2, time=0, xmin=0, xmax=1, ymin=0, ymax=5, xlab="", ylab="", log="", col=c("red", "blue"), lwd=c(2, 1), addone=FALSE, positive=FALSE, nvar=FALSE, add=FALSE, ...) {  
  dots <- list(...)
  if (!is.null(dots)) {
  ...
  bifurcation_data <- data.frame(x = numeric(), y = numeric(), stable = character())
  if (!add) {
  ...
  FUN <- function(lastState, lastDom, step) {
    ...
    while (xmin < lastP & lastP < xmax & ymin < lastState[y] + One & lastState[y] < ymax) {
      ...
      if (attr(q, "steady") & sum(abs(newState - lastState)) / (1e-9 + sum(abs(lastState))) < 0.1) {
        ...
        bifurcation_data <<- rbind(bifurcation_data, data.frame(x = parms[x], y = newState[y] + One, stable = ifelse(dom < 0, "Stable", "Unstable")))
        preLastState <- lastState
        lastState <- newState
        lastDom <- dom
        lastP <- parms[x]
        ...
        if (abs(step) > actualStep / 100) step <- step / 2
          ...
          if (attr(q, "steady") & sum(abs(newState - lastState)) / (1e-9 + sum(abs(lastState))) > 0.001) {
            ...
            bifurcation_data <<- rbind(bifurcation_data, data.frame(x = parms[x], y = lastState[y] + One, stable = ifelse(lastDom < 0, "Stable", "Unstable")))
            bifurcation_data <<- rbind(bifurcation_data, data.frame(x = parms[x], y = newState[y] + One, stable = ifelse(dom < 0, "Stable", "Unstable")))
            step <- -step
            preLastState <- lastState
            lastState <- newState
  ...
  FUN(lastState = q0$y, lastDom = sign(max(Re(eigen(jacobian.full(y = q0$y, func = odes, parms = parms))$values))), step = actualStep)
  FUN(lastState = q0$y, lastDom = sign(max(Re(eigen(jacobian.full(y = q0$y, func = odes, parms = parms))$values))), step = -actualStep)
  options(warn = orgWarn)
  return(bifurcation_data)
}

# Call the continue function and get bifurcation data
bifurcation_data <- continue(eq, x = "k", y = "KE2", xmax = 1, ymax = 5)

# Filter out points that might cause visual artifacts
bifurcation_data <- bifurcation_data[!duplicated(bifurcation_data$x), ]
