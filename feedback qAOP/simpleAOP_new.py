...
plt.figure()
plt.plot(sorted_k_values, sorted_percentages, marker='o')
plt.xlabel('k')
plt.ylabel(f'Percentage of AOs > {threshold}')
plt.title(f'Percentage of AOs values above {threshold}')
plt.grid(True)
plt.savefig(f"{pathToOutput}{DATETIME}_percentage_AO_sc_above_{threshold}.pdf")
plt.show()

print(f"Percentage plot saved as {pathToOutput}{DATETIME}_percentage_AO_sc_above_{threshold}.pdf")