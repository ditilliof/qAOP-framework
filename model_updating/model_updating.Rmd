...
```{r}
#We now generate artificial data from the model by adding noise to the solutions of the system.
...
artdata <- data.frame(time = numeric(), MIE1 = numeric(),  MIE2 = numeric(), KE1 = numeric(), KE3 = numeric(), KE2 = numeric(), REPLICATE = numeric())

for (i in 1:length(finish)) {
  for (j in 1:nrep) {
    artdata <- rbind(artdata, data.frame(time = finish[i], MIE1 = out$MIE1[i] + rnorm(1, mean = 0, sd = std),
...
variable = "KE2"
ggplot() + theme_bw() +
  geom_point(data = artdata, aes(time, get(variable), color = as.factor(REPLICATE))) + 
  labs(x = "Time", y = variable, color = "Replicate number")

write.csv(artdata, paste0(output_folder,"/", "artdatanetwork", ".csv"))
...
data_list=list(
N = length(unique(artdata$time)),
t0 = 0,
ts = unique(artdata$time), #does not have to include t0
