...
# Define args_plot with acceptable arguments for the plot function
args_plot <- c("xlim", "ylim", "xlab", "ylab", "log", "main", "sub", "type", "ann", "axes", "frame.plot", "panel.first", "panel.last", "asp", "font.main", "font.sub", "font.lab", "font.axis", "font")


args_steady <- c("y", "func", "parms", "times", "positive", "jacobian", "bandwidth", "maxiter", "atol", "rtol", "hmax", "solver", "method", "hmin")

index <- function(var, varnames) {
  if (is.character(var)) {
    which(var == varnames)
  } else {
    var
...
continue <- function(state=inistate, parms=pars, odes=feedbackqAOP, step=0.1, x=1, y=2, time=0, xmin=0, xmax=1, ymin=0, ymax=1.1, xlab="", ylab="KE2 (a.u.)", log="", col=c("red","black","blue"), lwd= c(2,1,1), addone=FALSE, positive=FALSE, nvar=FALSE, add=FALSE, ...) {  
  ...
  if (!add) {
    ...
    par(mar=c(5, 6, 4, 2) + 0.1) # Adjusting the margins
    do.call('plot',c(list(1,1,type='n',xlim=c(xmin,xmax),ylim=c(ymin,ymax),xlab=xlab,ylab=ylab,log=log,cex.axis=2, cex.lab=3,font.main=font.main,font.sub=font.sub),dots[names(dots) %in% args_plot]))
  ...
  FUN <- function(lastState,lastDom,step) {
    ...
    while (xmin < lastP & lastP < xmax & ymin < lastState[y]+One & lastState[y] < ymax) {
      ...
      if (attr(q,"steady") & sum(abs(newState-lastState))/(1e-9+sum(abs(lastState))) < 0.1) {
        ...
        if (logx) lines(c(parms[x]/(1+step),parms[x]),c(lastState[y]+One,newState[y]+One), col=COL(lastState,dom+2),lwd=3)
        else lines(c(parms[x]-step,parms[x]),c(lastState[y]+One,newState[y]+One), col=COL(lastState,dom+2),lwd=3)
        preLastState <- lastState
        lastState <- newState
        lastDom <- dom
        lastP <- parms[x]
        ...
        if (abs(step) > actualStep/100) step <- step/2
          ...
          if (attr(q,"steady") & sum(abs(newState-lastState))/(1e-9+sum(abs(lastState))) > 0.001) {
            ...
            lines(c(parms[x],parms[x]),c(lastState[y]+One,middle+One), col=COL(lastState,lastDom+2),lwd=3)
            lines(c(parms[x],parms[x]),c(middle+One,newState[y]+One), col=COL(newState,dom+2),lwd=3)
            step <- -step
            preLastState <- lastState
            lastState <- newState
            lastDom <- dom
